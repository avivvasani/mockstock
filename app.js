...
import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, createUserWithEmailAndPassword, signInAnonymously, signOut } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-auth.js';
import { getFirestore, doc, getDoc, setDoc, updateDoc, onSnapshot, collection, query, where, runTransaction } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
const catsWrap = $('cats');
const tradeDlg = $('trade');
const tradeMsg = $('tradeMsg');

let user = null;
let market = { updatedAt:null, categories:[], stocks:[] }; // from Firestore
let portfolio = null; // { cashBalance, holdings[], transactions[] }
...
$('guest').onclick = async()=>{
  try{ await signInAnonymously(auth); }catch(e){ $('authMsg').textContent = e.message; }
};
...
async function ensureUserDoc(uid){
  ...
  if(!snap.exists()){
    await setDoc(ref,{
      ...
      holdings: [], // {id, qty, avg}
      transactions: [] // {ts, id, side, qty, px}
    });
  }
}
...
function subscribeMarket(){
  ...
  unsubscribeMarket = onSnapshot(ref,(snap)=>{
    if(snap.exists()){
      market = snap.data();
      renderMarket();
    }
  ...
  for(const s of list){
    ...
    card.innerHTML = `
      ...
      <div class="flex gap-2">
        <button class="btn btn-accent" data-id="${s.id}" data-price="${s.price}">Trade</button>
      </div>
    `;
    marketGrid.appendChild(card);
  }
  marketGrid.querySelectorAll('button[data-id]').forEach(btn=>{
  ...
  try{
    ...
    await runTransaction(db, async (tx)=>{
      ...
      if(side==='BUY'){
        ...
        const h = holdings[idx];
        const newQty = h.qty - qty;
        if(newQty===0){ holdings.splice(idx,1); } else { holdings[idx] = { ...h, qty:newQty }; }
        cash += tcost;
      }
