...
func main() {
	...
	for _, tableName := range tables {
		...
		for rows.Next() {
			...
			histQ := fmt.Sprintf(`SELECT price FROM "%s" WHERE timestamp <= ? ORDER BY timestamp DESC LIMIT 2`, tx.Stock)
			priceRows, err := histDB.Query(histQ, tx.Timestamp)
			...
			if err != nil {
				...
				for priceRows.Next() {
					var p float64
					if err := priceRows.Scan(&p); err == nil {
				...
				if len(prices) == 0 {
					...
					if !priceMatched {
						slippage := math.Abs(tx.Price - prices[0]) / prices[0]
						if slippage > SLIPPAGE_PCT {
							globalMismatchCount++
							summary.Anomalies++
							allDiscrepancies = append(allDiscrepancies, DiscrepancyLog{tableName, tx.ID, tx.Timestamp, tx.Stock, fmt.Sprintf("$%.2f", tx.Price), fmt.Sprintf("$%.2f", prices[0]), "HIGH_SLIPPAGE/MANIPULATION"})
						} else if tx.Price != prices[0] {
							globalMismatchCount++
							summary.Anomalies++
							allDiscrepancies = append(allDiscrepancies, DiscrepancyLog{tableName, tx.ID, tx.Timestamp, tx.Stock, fmt.Sprintf("$%.2f", tx.Price), fmt.Sprintf("$%.2f", prices[0]), "MINOR_PRICE_MISMATCH"})
						}
					}
				}
			}
